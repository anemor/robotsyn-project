...
def plot_all(all_p, all_r, detections, subtract_initial_offset):
    ...
    fig, ax=plt.subplots(3,2, sharex = True, sharey = False, constrained_layout=True)
    fig.suptitle("Reprojection Error distribution", fontsize=14)


    plt.setp(ax,xlim=(0,200),ylim=(0,45))
    ax[0,0].hist(reprojection_errors, range = (0, 200), bins=40, color='rebeccapurple')
...
# Set common labels
    ...
    # ax[0,0].xlabel('Reprojection error (px)')

    ax[0,1].hist(reprojection_errors_wrist, range = (0, 200), bins=40, color='rebeccapurple')
    ax[0,1].set_title("Wrist", fontsize=9)
    # ax[0,1].set_xlabel('Reprojection error (px)', fontsize=8)
    # ax[0,1].set_ylabel('Frequency', fontsize=8)
    # ax[0,1].ylabel('Frequency')
    # ax[0,1].xlabel('Reprojection error (px)')
    # plt.ylabel('Frequency')
    # plt.xlabel('Reprojection error (pixels)')

    ax[1,0].hist(reprojection_errors_first, range = (0, 200), bins=40, color='rebeccapurple')
    ax[1,0].set_title("First", fontsize=9)
    # ax[1,0].set_xlabel('Reprojection error (px)', fontsize=8)
    # ax[1,0].set_ylabel('Frequency', fontsize=8)
    # ax[1,0].ylabel('Frequency')
    # ax[1,0].xlabel('Reprojection error (px)')
    # plt.ylabel('Frequency')
    # plt.xlabel('Reprojection error (pixels)')


    ax[1,1].hist(reprojection_errors_second, range = (0, 200), bins=40, color='rebeccapurple')
    ax[1,1].set_title("Second", fontsize=9)
    # ax[1,1].ylabel('Frequency')
    # ax[1,1].xlabel('Reprojection error (px)')
    # plt.ylabel('Frequency')
    # plt.xlabel('Reprojection error (pixels)')

    ax[2,0].hist(reprojection_errors_third, range = (0, 200), bins=40, color='rebeccapurple')
    ax[2,0].set_title("Third", fontsize=9)
    # ax[2,0].ylabel('Frequency')
    # ax[2,0].xlabel('Reprojection error (px)')
    # plt.ylabel('Frequency')
    # plt.xlabel('Reprojection error (pixels)')

    ax[2,1].hist(reprojection_errors_fingertip,range = (0, 200), bins=40, color='rebeccapurple')
    ax[2,1].set_title("Fingertip", fontsize=9)
    # ax[2,1].ylabel('Frequency')
    # ax[2,1].xlabel('Reprojection error (px)')
    # plt.ylabel('Frequency')
    # plt.xlabel('Reprojection error (pixels)')
    fig.supxlabel('Reprojection error (px)')
